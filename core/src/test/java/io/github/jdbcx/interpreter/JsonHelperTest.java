...
public class JsonHelperTest {
    ...
    public void testExtractFromInputStream(String path, String expectedValue) throws IOException {
        ...
        Assert.assertEquals(JsonHelper.extract(new ByteArrayInputStream(bytes), null, path), expectedValue);
        Assert.assertEquals(JsonHelper.extract(new ByteArrayInputStream(bytes), StandardCharsets.US_ASCII, path),
                expectedValue);
    }

    @Test(dataProvider = "pathAndValue", groups = { "unit" })
    public void testExtractFromReader(String path, String expectedValue) throws IOException {
        Assert.assertEquals(JsonHelper.extract(new StringReader(json), path), expectedValue);
    }
