...
ARG JDBCX_VERSION=0.2.0

# Environment variables
ENV LANG="en_US.UTF-8" LANGUAGE="en_US:en" LC_ALL="en_US.UTF-8" TERM=xterm \
    JDBCX_USER_ID=1000 JDBCX_USER_NAME=jdbcx JDBCX_VERSION=${JDBCX_VERSION:-0.2.0} \
    JDBCX_CONFIG=/etc/jdbcx/config.properties
...
RUN locale-gen en_US.UTF-8 \
    ...
    && useradd -r -Md / -s /bin/bash -u $JDBCX_USER_ID -g $JDBCX_USER_ID $JDBCX_USER_NAME \
