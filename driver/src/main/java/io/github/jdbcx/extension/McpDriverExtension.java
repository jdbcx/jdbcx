...
import io.github.jdbcx.interpreter.McpInterpreter;

public final class McpDriverExtension implements DriverExtension {
    static final class ActivityListener extends AbstractActivityListener {
        ActivityListener(QueryContext context, Properties config) {
            super(new McpInterpreter(context, config), config);
        }
    }
    ...
    public String getDescription() {
        return "Extension to interactive with MCP server";
    }

    @Override
    public String getUsage() {
        return "{{ mcp(cmd=npx, args=-y @modelcontextprotocol/server-everything, tool=echo): {\"message\": \"hello\"} }}";
    }
