...
import io.github.jdbcx.interpreter.PrqlInterpreter;
...
public class PrqlDriverExtension implements DriverExtension {
    static final class ActivityListener extends AbstractActivityListener {
        ActivityListener(QueryContext context, Properties config) {
            super(new PrqlInterpreter(context, config), config);
        }
    }

    @Override
    public List<String> getAliases() {
        return Collections.singletonList("prqlc");
    }

    @Override
    public List<Option> getDefaultOptions() {
        return PrqlInterpreter.OPTIONS;
    }
